...
#include <pybind11/embed.h>   // ← תיקון: חסר — scoped_interpreter מוגדר כאן
...
// ─────────────────────────────────────────────
// 2. המרת dict Python → unordered_map<uint64_t, Node>
// ─────────────────────────────────────────────
...
graphFromPyDict(const py::dict& py_nodes, const py::list& py_edges)
{
    ...
    // ── קשתות → מוסיפות לרשימת סמיכויות של source ──────────────────────────
...
// ─────────────────────────────────────────────
// 3. ממשק ציבורי – נקרא מ-NavigationManager
//    מחזיר: (myMap, source_node, target_node)
...
{
        // מאתחל Python רק אם עוד לא פעיל (בטוח לקריאה חוזרת)
            if (!Py_IsInitialized()) {
    ...
    try {
        // נתיב מוחלט ל-MapLive — PROJECT_SOURCE_DIR מוגדר ב-CMakeLists.txt
