...
// -----------------------------------------------------------------------
//  ObjectEntry — מה שה-Tracker שולח בכל פריים
//    updated — אובייקטים שנראו בפריים הזה (חדשים + עדכון)
//    removed — track_id של אובייקטים שהתיישנו ויש למחוק
// -----------------------------------------------------------------------
struct TrackerFrame {
...
public:
    ...
    // עדכון מיקום חלון הגריד כשהרובוט זז
    void shiftOrigin(float new_robot_x, float new_robot_y);

    // בניית שכבת ליידר (FREE/LETHAL) — רץ בכל פריים ליידר
    void buildFromOctomap();

    // עדכון מצטבר של שכבת אובייקטים
    void updateObjects(
    ...
    // ניפוח גלובלי על שכבת הליידר בלבד
    void inflationPass();
    ...
    // מיזוג lidarLayer + objectLayer → costMapFused
    void fuseMaps();
...
private:
    ...
    // מוצא Edge בין שני צמתים בגרף
    static const Edge* findEdgeBetween(
        const std::unordered_map<uint64_t, Node>& graph,
        uint64_t from_id,
        uint64_t to_id);

    // מצייר עיגול ב-routeLimitLayer
    void paintCircleOnRouteLayer(
        const Eigen::Vector2f& center_pos,
        float                  radius_m,
        uint8_t                cost);

    // מנקה את טביעת הרגל של אובייקט משכבת objectLayer
    void eraseObjectFootprint(ObjectEntry& entry);

    // מצייר גוף + ניפוח + שדה קדמי ל-objectLayer
    void paintObject(const TrackedObject& obj, ObjectEntry& entry);

    // --- פונקציות ציור פנימיות (מקבלות set לאיסוף אינדקסים ייחודיים) ---
