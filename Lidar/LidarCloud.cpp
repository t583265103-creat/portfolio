...
//מקבל קלט מ3 ענני לידאר ומאחד אותם
...
void LidarCloud::downSampleGlobalCloud() {
    ...
    sor.setInputCloud(LidarCloudGlobal); // ����� ����� �� �-Ptr
...
//מצמצם את ענן הנקודות לגודל קבוע    
...
void LidarCloud::ClusteringCloudToObjects() 
{
    ...
    ec.setClusterTolerance(roiParms.maxDistanceBetweenPoints); //קובע כמה מרחק מקסימלי יהיה בין אובייקט לאובייקט
    ec.setMinClusterSize(roiParms.minPointsInCloud);     //קובע כמה מינימום נקודות יהיה באוביקט
    ec.setMaxClusterSize(roiParms.maxPointsInCloud);//קובע כמה מקסימום נקודות יהיה באוביקט
    ec.setSearchMethod(tree);   //משתמש בK-tree
