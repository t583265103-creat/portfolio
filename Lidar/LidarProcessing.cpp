...
// 7. Project Headers
#include "LidarProcessing.hpp" // מומלץ לכלול קודם כל את ההדר של המחלקה הנוכחית
...
// תיקון: הוספת הקידומת LidarProcessing:: לפונקציה process
...
void LidarProcessing::processLidar(std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr>& raw_clouds) {
    // מניעת קריסה בגישה לאינדקסים אם הוקטור ריק או חלקי
...
float LidarProcessing::calculateConfidence(const LidarObject& LO, const ROI_box& params) {
    // הגנה מפני קריסה אם ה-cloud בתוך LO אינו מאותחל
