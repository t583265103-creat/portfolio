...
namespace {
constexpr double GPS_PERIOD_SEC  = 0.10;  // 10Hz
constexpr double SLOW_PERIOD_SEC = 0.50;  // 2Hz: LiDAR / Camera / Radar
...
// מדפיס פעם בכל N קריאות כדי לא להציף את Webots console
...
void SensorProducerWebots::enableDevices()
{
    ...
    // Ensure we don't push slow sensors (LiDAR/Camera/Radar) immediately
    // — start the slow-push timer now so the first push happens after the
    // sampling period (SLOW_PERIOD_SEC).
...
void SensorProducerWebots::pushImu(double t)
{
    ...
    // כרגע raw_mag_x/y/z מחזיקים roll/pitch/yaw.
    if (finite3(rpy)) {
...
SensorProducerWebots::lidarToCloud(WbDeviceTag lidar)
{
    ...
    // Use device max range to detect no-hit values reported by the lidar
    ...
    const double MIN_VALID_RANGE = 0.01; // meters
    ...
    for (int i = 0; i < n; ++i) {
        ...
        // treat near-zero ranges as invalid (no hit)
        if (r <= MIN_VALID_RANGE) {
            ++zero_count; ++skipped_count; continue;
        }

        // If device reports a known max range, treat max-range returns as misses
    ...
    // Debug summary for ranges -> XYZ conversion
...
void SensorProducerWebots::pushLidar(double t)
{
    ...
    // Front-center sector min range for emergency stop (±30° horizontal, robot frame).
    // tan(30°) ≈ 0.5774; only points forward of sensor (x > 0) in the central cone.
    {
...
cv::Mat SensorProducerWebots::cameraToMat(WbDeviceTag camera)
{
    ...
    // Compute simple brightness stats while converting (Rec.601 luma)
...
std::string SensorProducerWebots::makeNmea(double t) const
{
    ...
    // Webots world frame (Z-up):
    //   v[0] = East,  v[1] = North (negative = South),  v[2] = Up (altitude)
    const double east_m  = x;   // v[0] = East
    const double north_m = y;   // v[1] = North
    const double alt_m   = z;   // v[2] = Up/altitude
