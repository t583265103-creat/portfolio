...
struct ImuMeasurement {
    ...
    // טמפרטורה (בהרבה חיישנים הרגיסטר הזה "תקוע" פיזית באמצע)
    int16_t raw_temperature; 

    // 2. ג'יירוסקופ (3 צירים)
    int16_t raw_gyro_x;
    int16_t raw_gyro_y;
    int16_t raw_gyro_z;
    
    // 3. מגנטומטר / מצפן (3 צירים) --> התוספת שהופכת אותו ל-9 צירים!
    int16_t raw_mag_x;
    int16_t raw_mag_y;
    int16_t raw_mag_z;
};
...
public:
    ...
    void addCameraUpdate(double ts, const cv::Mat& frame) {
        // התיקון הקריטי: יצירת עותק עמוק כדי שהמצלמה לא תדרוס את הזיכרון
