...
void ThreadGrafFector::SecurityWall()
{
    ...
    for (const auto& [id, track_ptr] : tracked_map)
    {
        ...
        // בדיקת כיוון:
        // אם dot > 0 — האובייקט נע בערך לכיוון הרובוט
    ...
    // אם אני בצומת עם רמזור אבל לא ראיתי רמזור — עדיף לעצור בזהירות
    if (atCrossing && currentNode && currentNode->has_traffic_light && !trafficLightSeen) {
        trafficBlocked = true;
    }

    // מעבר חציה בלי רמזור:
    // בשלב בסיסי נעצור רק אם יש אובייקט דינמי מסוכן.
...
void ThreadGrafFector::ConsumerProcessingFrame() {
    ...
    try {
        ...
        if (matched_frame) {
            bool has_lidar_data = false;  // set inside lock, used outside for runDLite
            bool has_gps_data   = false;  // set inside lock, used outside for POSE_SYNC_DEBUG
            {
            std::lock_guard<std::mutex> lock(matched_frame->frame_mutex);

            // determine what data is available — null-safe + list-empty check
            ...
            // sensorfusion must exist — it's always initialized in FramePointers ctor,
            // but guard anyway since fusion cannot proceed without it
            ...
            // object fusion — partial: run only sensors that have data
            ...
            // ICP — requires LiDAR and a previous frame
            ...
            }  // release frame_mutex before runDLite — OctomapAdapter re-locks it internally
            ...
            // Save frames older than 5 s to the database and remove from memory
    ...
    } catch (const std::exception& e) { // 2. סגירת ה-try ותחילת ה-catch
...
void ThreadGrafFector::Optimization() {
    ...
    // save pre-optimization state of the last node — delta will be pure graph correction
    ...
    // graph correction = what the optimizer changed in the last node (not temporal drift)
    ...
    // guard 1: positions are finite and within physical bounds
    ...
    // guard 2: delta magnitude — no clamp, reject entirely if too large or non-finite
    ...
    task.execute = [graph_delta, post_opt_P,
        ...
        // only X/Y position correction — zero everything else
        delta(idx_yaw)  = 0.0;
        delta(idx_vx)   = 0.0;
        delta(idx_vyaw) = 0.0;
        delta(idx_ax)   = 0.0;
