...
void SensorProducer::init() {
    // מזמנים את התהליכונים. ה-WorkerManager יריץ אותם בתוך לולאה.
...
void SensorProducer::ProducerLidar() {
    ...
    auto generate_clouds = [&](bool print) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            p.z = 0.1f * (i % 3);
            p.intensity = 50.0f; // ensure intensity above ROI minIntensity
    ...
    if (!lidar_mock_mode) {
        ...
        if (!front_ok || !right_ok || !left_ok) {
            ...
            generate_clouds(true); // print generation log once when switching
        }
    }
