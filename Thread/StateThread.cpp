...
void StateThread::init() {
    is_running_ = true;
    // יצירת התהליכון והפעלת פונקציית הלולאה
...
void StateThread::stop() {
    ...
    // דחיפת משימת דמה (Dummy Task) כדי לשחרר את ה-pop() החוסם אם התור ריק
...
void StateThread::stateMainLoop() {
    ...
    while (is_running_) {
        ...
        // 2. חישוב dt — תקף לכל מודל (חריגה ב-dt מסוכנת גם לרובוט וגם לאובייקטים, אבל
        //    הבדיקה הזאת משפיעה רק על ה-predict של ה-Robot EKF שמתבצע כאן; ה-predict
        //    של אובייקטים רץ בנפרד ב-SensorFusionManager.cpp ולא נוגעים בו.
        ...
        constexpr double MAX_VALID_DT = 0.5;    // dt גדול מזה חריג — מדלגים על predict כדי לא "לקפוץ" ב-X/Y/Vx
        constexpr double MAX_ABS_VX           = 2.0;   // גבול פיזי סביר למהירות הכיסא [מ"ש] — רק ל-Robot EKF
        constexpr double VX_DECAY_PER_PREDICT = 0.999; // משיכה עדינה ל-0 כשאין תיקון Encoder תדיר — רק ל-Robot EKF
        ...
        // 3. Access EKF under lock for predict; do not hold lock during task execution to avoid deadlocks
        {
            std::unique_lock<std::mutex> lock(state_queue.getEKFMutex());

            auto& ekf = state_queue.getEKF();

            // Validate EKF state size
            ...
            if (dt_valid) {
                ...
                // Robot-only: Vx ב-predict מוגבל ב-decay+clamp כדי שלא יברח כשה-Encoder
                // (מקור האמת היחיד לתיקון Vx) לא מתקן אותו בתדירות מספקת. לא משפיע על
                // EKF של אובייקטים — אלה נחזים בנפרד ב-SensorFusionManager.cpp.
            ...
            // release lock before executing the task to avoid deadlocks if task tries to lock same mutex
            lock.unlock();
        }

        // 4. Execute task under EKF mutex — prevents races with getStateVectorSafe()
        ...
        try {
            ...
            // setTimeLastUpdate has its own lock — call after releasing exec_lock
