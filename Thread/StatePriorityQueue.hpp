...
private:
    ...
    // חייב להיות mutable כי פונקציות const גם צריכות לנעול אותו לצורך קריאה בטוחה
...
public:
    ...
    // קריאה בטוחה ל-StateVector
    Eigen::VectorXd getStateVectorSafe() const {
        std::lock_guard<std::mutex> lock(m_ekf_mutex);
        return StateVector.x;
    }

    // השארתי את השם הישן כדי שקוד קיים לא יישבר
    Eigen::VectorXd getStateVector() const {
        return getStateVectorSafe();
    }

    // שימי לב: הפונקציה הזאת מחזירה reference ולכן היא לא thread-safe בפני עצמה.
    // אם את משתמשת בה לעדכון EKF מתוך Thread אחר, צריך לנעול מבחוץ.
