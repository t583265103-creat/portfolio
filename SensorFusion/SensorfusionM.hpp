...
protected:
    ...
    ///פונקציות טיפול באוביקט לאחר שידוך
   ...
   virtual void increaseUncertainty(OutputObject& Object)
{
    ...
    ///פונקציות של בניית מטריצת העלויות
 std::vector<std::vector<double>> CreateAssociationMatrix(std::vector<RawInput>& sensor_list, std::vector<OutputObject>& object_list)
    {
        ...
        // בדיקת קצה: אם אין זיהויים או מסלולים, נחזיר מטריצה ריקה
     ...
     float calculate_cost(RawInput& sensor_in, OutputObject& object)
    {
        ...
        if constexpr (std::is_same_v<OutputObject, SensorFusionObject> || std::is_same_v<OutputObject, TrackedObject>) {
            obj_pos = object.filter.x.head<3>().cast<float>(); // מיקום מתוך הפילטר של האובייקט
        } else {
        ...
        // 3. Dimensions Gating
        if constexpr (!std::is_same_v<RawInput, RadarObject>) { // נריץ רק אם זה לא רדאר!
            float max_dev = 0.6f;
     ...
     float calculaterect(const cv::Rect& rectA, const cv::Rect& rectB)
    {
        ...
        // הגנה מפני חלוקה ב-0 במקרה שאין חפיפה כלל
        if (interArea <= 0) {
            return 0.0f;
        }
    ...
    {
            // 1. חישוב הפרש הזמן (בשניות)
        double delta_t = sensor.timestamp - Object.timestamp;
        
        // 2. קביעת קבוע הדעיכה (למשל 2.0 - כדאי לכוונן בניסויים)
        const float lambda = 2.0f;
        
        // 3. חישוב המעריך (exponent)
        float exponent = -1.0f * (lambda * static_cast<float>(delta_t) * sensor.confidence);
        
        // 4. חישוב אלפא לפי הנוסחה: alpha = 1 - e^(exponent)
        float alpha = 1.0f - std::exp(exponent);
        
        // 5. הבטחת טווח תקין (Safety Clamp)
        // מונע מאלפא להיות קטנה מדי (כדי שתמיד תהיה השפעה למדידה)
     ...
     double calculateMahalanobis(RawInput& sensor_in, OutputObject& object)
    {
        ...
        // // אם זו מצלמה, אנחנו יכולים להוסיף "פקטור ביטחון" זמני רק לחישוב העלות
        // if (sensor_in.sensor_type == SensorType::Camera) {
        //     double dist = object.filter.x(Config::StateZ);
        //     R_temp *= (1.0 + (dist * 0.05)); 
        // }
     ...
     double calculateIoU(RawInput& sensor_in, OutputObject& object)
    {
        ...
        // א. שליפה מתוך החיישן הנכנס (שימוש ב-RawInput כטיפוס)
        ...
        // ב. שליפה מתוך אובייקט המעקב / SensorFusionObject
        ...
        // בדיקת תקינות לפני חישוב
        if (cam_rect.empty()) return 0.0;

        // החישוב יתבצע רק אם יש לנו ענן נקודות תקין
    ...
    {
    // 1. בדיקה שהענן אינו ריק
    if (!objectCloud || objectCloud->empty()) {
        return cv::Rect();
    }

    // 2. שליפת קונפיגורציית החיישנים מתוך ה-ConfigManager (מבוסס על ה-YAML שלך)
    // בדוגמה זו נניח שאנו עובדים עם החיישנים הקדמיים ("lidar_front" ו-"camera_front")
    auto lidarConfig = ConfigManager::getLidar("front_lidar");
    auto cameraConfig = ConfigManager::getCamera("rgb_camera");

    // 3. חישוב המטריצה והווקטור היחסים (ממערכת צירים לידאר למערכת צירים מצלמה)
    // החישוב מתבצע על ידי הכפלת המטריצה ההפוכה (Transpose) של המצלמה בנתוני הלידאר
    ...
    // 4. סינון נקודות והכנת מערך נקודות תלת-ממדיות להקרנה
    std::vector<cv::Point3f> points3D;
    points3D.reserve(objectCloud->size());

    for (const auto& pcl_point : objectCloud->points) {
        // העברה זמנית של הנקודה למערכת הצירים של המצלמה כדי לוודא שהיא מלפנים
        Eigen::Vector3f pt_lidar(pcl_point.x, pcl_point.y, pcl_point.z);
        Eigen::Vector3f pt_camera = R_lidar2cam * pt_lidar + T_lidar2cam;

        // סינון קריטי: נקודות שנמצאות מאחורי המצלמה (Z <= 0) יגרמו לעיוותים קשים במתמטיקה של ההקרנה,
        // לכן נשמור רק נקודות שנמצאות פיזית מול עדשת המצלמה (Z > 0.1 מטר)
        if (pt_camera.z() > 0.1f) {
            points3D.push_back(cv::Point3f(pcl_point.x, pcl_point.y, pcl_point.z));
        }
    }

    // אם לאחר הסינון לא נשארו נקודות מול המצלמה, נחזיר מלבן ריק
    if (points3D.empty()) {
        return cv::Rect();
    }

    // 5. המרת משתני הטרנספורמציה מ-Eigen ל-OpenCV
    cv::Mat R_cv, tvec;
    cv::eigen2cv(R_lidar2cam, R_cv);
    cv::eigen2cv(T_lidar2cam, tvec);

    // המרת מטריצת סיבוב (3x3) לווקטור סיבוב (rvec בגודל 1x3) באמצעות אלגוריתם רודריגז כפי ש-projectPoints דורש
    cv::Mat rvec;
    cv::Rodrigues(R_cv, rvec);

    // 6. המרת מטריצת הכיול הפנימית K מתוך קובץ ה-Constants.hpp שלך
    Eigen::Matrix3f K_eigen = Config::getK();
    cv::Mat cameraMatrix;
    cv::eigen2cv(K_eigen, cameraMatrix);

    // יצירת וקטור מקדמי עיוות (Distortion Coefficients) המכיל אפסים (בהנחה שאין עיוות עדשה או שהיא כבר מיושרת)
    cv::Mat distCoeffs = cv::Mat::zeros(5, 1, CV_32F);

    // 7. ביצוע ההקרנה לדו-ממד (שימוש בסדר הפרמטרים הנכון: cameraMatrix ואז distCoeffs)
