...
// תיקון: זוהי מתודה של המחלקה, לכן חייב להוסיף SensorFusionManager::
...
TrackedObject to;
    ...
    to.id = ++my_static_id; 
    to.Z = sf.Z.cast<double>(); // המרה מפורשת
    to.H = sf.H.cast<double>(); // המרה מפורשת
    to.R = sf.R.cast<double>(); // המרה מפורשת
    to.F = sf.F.cast<double>(); // המרה מפורשת
    
    to.filter.x.setZero();
    to.filter.x.head<3>() = sf.position().template cast<double>();        
    
    if (sf.has_radar) {
        // וודא שאתה ממיר במפורש גם כאן
...
void SensorFusionManager::updateObject(SensorFusionObject& sensor_object, TrackedObject& track) {
    ...
    // Predict: קידום ווקטור המצב לפי מודל מהירות קבועה
    ...
    // Update: עדכון EKF עם מדידה משולבת מכל החיישנים הפעילים
    if (track.Z.rows() > 0) {
