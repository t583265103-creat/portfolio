...
{
    // שלב 5: נסה לטעון מודל מאומן על Webots — אם קיים, השתמש בו
...
std::vector<CameraObject> CameraProcessing::process(cv::Mat& input,double tss) {
    ...
    // שלב 1: שמירת raw frames לדאטאסט — כל הרצה ממשיכה מהframe האחרון
    {
        static int ds_saved = -1;  // -1 = לא אותחל עדיין
    ...
    // שלב א': זיהוי גולמי באמצעות YOLO (משתמש בתמונה שמתקבלת בקלט)
    std::vector<DetectedItem> raw_detections = yolo(input);

    // שלב ב': סנכרון ויצירת אובייקטים מעובדים לתצורה הסופית
...
std::vector<DetectedItem> CameraProcessing::yolo(const cv::Mat& input) {
    ...
    if (input.empty()) {
        ...
        return results; // מתוקן: מחזיר וקטור ריק במקום קריסה/שגיאת טיפוס
    }
    ...
    // עיבוד מקדים: YOLOv8 צריך תמונה בגודל 640x640 ונרמול צבעים
    if (should_log)
    ...
    // פענוח המטריצה הגולמית של YOLOv8 [1 x 84 x 8400]
    ...
    if (detMat.dims == 3) {
        detMat = detMat.reshape(1, detMat.size[1]); // משנה ל- [84 x 8400]
        cv::transpose(detMat, detMat);              // הופך ל- [8400 x 84]
    }

    // שלב 5: classNames ו-relevantClasses לפי המודל הנטען
    std::vector<std::string> classNames;
    std::set<int>            relevantClasses;
    float                    detConf;

    if (usingCustomModel) {
        // מודל מאומן על Webots — 7 מחלקות בלבד, סף confidence גבוה
        classNames      = {"person", "car", "bus", "motorcycle", "bicycle", "traffic light", "stop sign"};
        relevantClasses = {0, 1, 2, 3, 4, 5, 6};
        detConf         = 0.5f;
    } else {
        // yolov8n בסיסי — COCO 80 מחלקות, סף נמוך לסימולציה
        classNames = {
    ...
    // מעבר על 8400 המיקומים האפשריים של YOLOv8
    for (int i = 0; i < detMat.rows; ++i) {
        // שימוש בפוינטר ישיר לשורה i - מונע שגיאות זיכרון וקריאת ערכי זבל!
        const float* data = detMat.ptr<float>(i);

        // פלטי הקלאסים מתחילים מאינדקס 4 ועד הסוף
        cv::Mat scores(1, detMat.cols - 4, CV_32F, const_cast<float*>(data + 4));
        cv::Point classIdPoint;
        double maxScore;
        cv::minMaxLoc(scores, 0, &maxScore, 0, &classIdPoint);

        // סף 0.01 — Webots לא פוטוריאליסטי ו-YOLO נותן confidence נמוך לסצנות מדומות
        // עצמים מתחת לסף 0.5 מסומנים "unknown" אבל עדיין נשלחים למערכת
        if (maxScore > detConf && relevantClasses.count(classIdPoint.x) > 0) {
            ...
            if (w < 10 || h < 10) continue;           // קטן מדי → רעש
            if (w > input.cols * 0.95f) continue;     // גדול מדי → שגיאה
    ...
    // בניית רשימת ה-DetectedItem המסונכרנת
    for (int idx : indices) {
         DetectedItem item;
         item.box = boxes[idx];
...
CameraObject CameraProcessing::createObject(const DetectedItem& raw_detection) {
    ...
    // 2. מיקום בתמונה (2D) - חישוב ה-Centroid מתוך ה-box הקיים
    ...
    // 3. חישוב Confidence דינמי
    double detection_noise = 1.0 - raw_detection.confidence; // סונכרן מ-score ל-confidence
    ...
    // 6. קביעת תקינות סופית
    co.isValid = (co.confidence > 0.4);
    co.bounding_box = raw_detection.box;
    // שליחה לעיבוד/שמירה ב-CameraData כפי שהגדרת
...
float CameraProcessing::getDepthZ(const cv::Rect& box) {
    ...
    // 1. נקודת המגע עם הרצפה בפיקסלים (מרכז תחתית קופסת החסימה)
    ...
    // 2. יצירת וקטור כיוון (קרן) במרחב המצלמה
    ...
    // 3. שליפת מטריצת הרוטציה של המצלמה לעולם
    auto config = ConfigManager::getCamera("rgb_camera"); 
    Eigen::Matrix3f R = config.rotation; 

    // 4. סיבוב וקטור הקרן למערכת קואורדינטות העולם
    Eigen::Vector3f ray_world = R * ray_cam;

    // הגנה: רק קרניים הפונות כלפי מטה (y חיובי בקואורדינטות מצלמה) יפגשו את הרצפה
    if (ray_world.y() < 0.01f) {
        return -1.0f;
    }

    // 5. חישוב העומק (camera_height / ray_y — עובד עם מטריצת הרוטציה הנוכחית)
...
float CameraProcessing::calculatePhysicalHeight(const cv::Rect& box) {
    ...
    if (Z < 0) return 1.7f; // דיפולט אם החישוב נכשל
...
float CameraProcessing::calculatePhysicalWidth(const cv::Rect& box) {
    float Z = getDepthZ(box);
    if (Z < 0) return 1.8f; // דיפולט

    float fx = K(0, 0);
    return (box.width * Z) / fx;
}
float CameraProcessing::calculatePhysicalLength(float physical_width, float physical_height) {
    // הגנה בסיסית מפני ערכים לא חוקיים
    if (physical_width <= 0 || physical_height <= 0) {
        return 2.0f; // ערך ברירת מחדל כללי במטרים
    }

    // 1. חישוב יחס הצורה הפיזיקלי של האובייקט
    float aspect_ratio = physical_width / physical_height;

    // 2. חישוב המכפיל הדינמי
    float multiplier = aspect_ratio * 2.0f;

    // 3. חסימת המכפיל בטווח בטוח כדי שהאורך לא יצא עצום או אפסי
    // לפחות פי 0.5 מהרוחב (עבור אנשים/עצמים צרים) ומקסימום פי 3.0 מהרוחב (עבור רכבים)
...
cv::Point3f CameraProcessing::transformTo3D(cv::Point2f centroid, const cv::Rect& box) {
    ...
    // 1. נשתמש במרכז התיבה בפיקסלים כדי למצוא את מיקום מרכז האובייקט במרחב
    float pixel_center_x = box.x + (box.width / 2.0f);
    float pixel_center_y = box.y + (box.height / 2.0f);

    // 2. נרמול קואורדינטות הפיקסל למערכת המצלמה
    float x_norm = (pixel_center_x - cx) / fx;
    float y_norm = (pixel_center_y - cy) / fy;

    // 3. בניית וקטור 3D טהור במערכת הקואורדינטות של המצלמה (Camera Frame)
    ...
    // 4. שליפת טרנספורמציית המצלמה המלאה (Extrinsics)
    auto config = ConfigManager::getCamera("rgb_camera"); 
    Eigen::Affine3f transform = Eigen::Translation3f(config.translation) * config.rotation;

    // 5. המרה אבסולוטית למערכת העולם (סיבוב + הזזה בפעולה אחת)
