...
// מבנה הנתונים הגולמי שיוצא מרשת ה-YOLO
struct DetectedItem {
    std::string label;     // למשל "person"
    float confidence;       // ציון הזיהוי (0.0 עד 1.0)
    cv::Rect box;           // המיקום בתמונה (x, y, רוחב, גובה)
};

class CameraProcessing : public SensorProcessing<cv::Mat, std::vector<CameraObject>> {
public:
    CameraProcessing(); 
    // הפונקציה הראשית שמממשת את ה-Pipeline של החיישן
    std::vector<CameraObject> process(cv::Mat& input, double ts) override;

protected:
    // פונקציות הצינור הפנימיות
    std::vector<DetectedItem> yolo(const cv::Mat& input);
    CameraObject createObject(const DetectedItem& raw_detection);  
    // פונקציות עזר גיאומטריות וטרנספורמציה (הצהרות חסרות שהיו חסרות לקומפילציה)
